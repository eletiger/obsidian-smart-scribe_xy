...
export default class AIMetadataPlugin extends Plugin {
	...
	private posToOffset(content: string, pos: { line: number; ch: number }) {
	...
	private async applyOptimizedText(
		file: TFile,
		editor: Editor,
		isPartial: boolean,
		originalSelection: string,
		selectionFrom: { line: number; ch: number } | null,
		selectionTo: { line: number; ch: number } | null,
		...
		if (isPartial) {
			...
			if (currentSelectedText === originalSelection) {
				...
				if (idx === -1) idx = currentValue.indexOf(originalSelection);
				if (idx === -1) {
